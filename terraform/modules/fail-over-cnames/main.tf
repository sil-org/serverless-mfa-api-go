...
resource "cloudflare_dns_record" "public_cname" {
  depends_on = [
    cloudflare_dns_record.primary_region_cname,
    cloudflare_dns_record.secondary_region_cname
  ]
  ...
  type    = "CNAME"
  content = local.primary_region_intermediate_domain # <-- This controls which region to use by default.
...
resource "cloudflare_dns_record" "primary_region_cname" {
  ...
  ttl     = 1 # ttl must be set to 1 when proxied is true
  type    = "CNAME"
  content = var.primary_api_gateway_domain
  zone_id = data.cloudflare_zone.this.id
}
